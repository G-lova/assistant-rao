...
experts_with_coords AS (
	...
	SELECT 
		...
		ROUND(CASE 
			WHEN u.countAcceptedExpertises_lastYear > 0 AND u.countAcceptedExpertises_lastYear <= 5 THEN 0.25
			WHEN u.countAcceptedExpertises_lastYear > 5 AND u.countAcceptedExpertises_lastYear <= 10 THEN 0.5
			WHEN u.countAcceptedExpertises_lastYear > 10 AND u.countAcceptedExpertises_lastYear <= 15 THEN 0.75
			WHEN u.countAcceptedExpertises_lastYear > 15 THEN 1
			ELSE NULL
		END, 3) AS criterion1,	
		ROUND(CASE 
			WHEN u.countTotalExpertises_lastYear > 0
			THEN (u.countTotalExpertises_lastYear - u.overdues) / u.countTotalExpertises_lastYear
			ELSE NULL
		END, 3) AS criterion2,
		ROUND(CASE 
			WHEN u.countTotalExpertises_lastYear > 0
		...
		END, 3) AS criterion3,
		CASE 
			WHEN (u.countTotalExpertises_lastYear + u.expert_declines) > 0
			THEN (u.countTotalExpertises_lastYear) / (u.countTotalExpertises_lastYear + u.expert_declines)
			ELSE 0
