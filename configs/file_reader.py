...
#     """Скачивает файл по прямой ссылке и читает его содержимое"""
...
#         return read_file(temp_path, url)
...
async def read_pptx_file(file_path: str) -> str:
    """
        ...
        if ext == '.xls':
            try:
                ...
                try:
                    ...
                    return await read_excel_with_libreoffice(file_path)
        elif ext == '.xlsx':
...
async def read_excel_with_libreoffice(file_path: str) -> str:
    """
...
async def process_archive(file_path: str, ext: str) -> str:
    """
    ...
    async def process_single_file(filename, data):
        ...
        try:
            ...
            text = await read_file(extracted_path, original_filename=filename)
            return f"=== {filename} ===\n{text}"
        except Exception as e:
            return f"=== {filename} ===\n[Ошибка чтения: {e}]"
        finally:
...
async def read_file(file_path: str, original_filename: str = None) -> str:
    """
    ...
    try:
        ...
        if ext == ".bin":
            real_extension = await detect_binary_file_type(file_path)
            ...
            if real_extension != ".bin":
                ...
                try:
                    result = await read_file(new_path, original_filename + real_extension)
                    os.unlink(new_path)
                    return result
                except Exception as e:
        ...
        if ext in [".txt", ".csv", ".json", ".html"]:
            ...
            text = await read_excel_file(file_path, ext)
            logger.info(f"Извлечён текст из Excel: {original_filename}, длина: {len(text)}")
            return text
        elif ext == ".pptx":
            text = await read_pptx_file(file_path)
...
async def detect_binary_file_type(file_path: str) -> str:
    """
